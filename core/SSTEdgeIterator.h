...
struct tmp_eb_new_fixed{
  ...
  class LFEdgeIterator : public EdgeIteratorBase {
    public:
      LFEdgeIterator(char* body_data,
                      char* property_data,
                      size_t body_num,
                      FileId_t fid=INVALID_File_ID, SequenceNumber_t newest_edge_ = 0)
          : body_data_(body_data),
            property_data_(property_data),
            body_num_(body_num),
            body_cursor_(body_data),
      ...
      VertexId_t dst_id() const override {
        return *(VertexId_t*)(body_cursor_);
      }

      Marker_t marker() const override {
        return (Marker_t)(*(int*)(body_cursor_ + 8 + 8));
      }

      SequenceNumber_t sequence() const override {
        return *(SequenceNumber_t*)(body_cursor_ + 8 );
      }

      bool is_out() const override {
        const uint32_t marker_meta =
            *(reinterpret_cast<const uint32_t*>(body_cursor_ + 8 + 8));
        return ((marker_meta >> 30) & 0x1) != 0;
      }

      uint8_t edge_type() const override {
        const uint32_t marker_meta =
            *(reinterpret_cast<const uint32_t*>(body_cursor_ + 8 + 8));
        return static_cast<uint8_t>((marker_meta >> 22) & 0xff);
      }
      ...
      EdgeProperty_t edge_data(int id) const override {
        if (!valid()) {
          ...
          return EdgeProperty_t(property_data_ + *(int*)(body_cursor_ + 8 + 8 + 4), *(int*)(body_cursor_ + 24 + 8 + 8 + 4) - *(int*)(body_cursor_ + 8 + 8 + 4));
        }
      }
      ...
      Slice edge_slice_data(int id) {
        if (!valid()) {
          ...
          return Slice(property_data_ + *(int*)(body_cursor_ + 8 + 8 + 4), *(int*)(body_cursor_ + 24 + 8 + 8 + 4) - *(int*)(body_cursor_ + 8 + 8 + 4));
        }
      }
